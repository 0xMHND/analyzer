...
int formatter_write( void * ptr, int type)
{
	...
	if (type == OCR_FUNCTIONS) {
		...
		for (int i = 0; i < blocks->d_count; i++) {
			if (snprintf(buf, BUF_SIZE, "destroy %ld %ld %ld\n", blocks->destroy[i].instr_count, blocks->destroy[i].depth, blocks->destroy[i].id) >= BUF_SIZE)
				printf("Warning: Buffer size exceeded in formatter_write()\n");
			if (fputs(buf, f) == EOF)
				fprintf(stderr, "Could not put \"%s\" in %s: %s.\n", buf, full_path, strerror(errno));
		}		
	...
	//else, if it's processed data, ignore for now
		...
		for(int i=0; i<master->size;i++){
			fprintf(f,	"Block #%d, ID#%ld\t", i, master->ptr[i]->id);
			fprintf(f,	"lifetime: %lf\t", (double) master->ptr[i]->lifetime_instructions);
			fprintf(f,	"instr. %lf" "seconds\n", (double)master->ptr[i]->lifetime_time);
		}
	...
	//else, if it's a memory analysis, ignore for now
		...
		snprintf(full_path, BUF_SIZE, "%s%s", LOG_PATH, "memory_analysis.out");
		if ((f = fopen(full_path, "w")) == NULL) {
			fprintf(stderr, "Could not open file %s: %s.\n", full_path, strerror(errno));
			return 1;
		}
